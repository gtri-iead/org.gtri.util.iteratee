...
class TraversableEnumerator[A](traversable : Traversable[A], chunkSize : Int = STD_CHUNK_SIZE) extends Enumerator[A] {
  ...
  class ContWithDefSize[A](current : Traversable[A]) extends Enumerator.State[A] {
    val status = new Status(StatusCode.CONTINUE,0,traversable.size - current.size, traversable.size)
    ...
    def step = {
      ...
      if(remaining.isEmpty) {
        ...
        Result(new ContWithDefSize(remaining), output = nextChunk)
      }
    }
  }
